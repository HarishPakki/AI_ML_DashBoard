// src/components/ExecutionTable.js
...
import { getPrediction } from '../predictionUtils'; // Import the prediction utility
...
const ExecutionTable = ({ reports }) => {
    ...
    useEffect(() => {
        if (Array.isArray(reports)) {
            ...
            // Call the prediction function with sample data
            const sampleData = reports.map(report => report.timestamp ? new Date(report.timestamp).getTime() : 0);
            getPrediction(sampleData).then(prediction => {
